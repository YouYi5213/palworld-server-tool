...
type DirectRconRequest struct {
	Command string `json:"command"`
}

// sendDirectRcon godoc
//
//	@Summary		Send Raw Rcon Command
//	@Description	Send a raw RCON command directly without UUID lookup
//	@Tags			Rcon
//	@Accept			json
//	@Produce		json
//	@Security		ApiKeyAuth
//	@Param			command	body		DirectRconRequest	true	"Raw Rcon Command"
//	@Success		200		{object}	MessageResponse
//	@Failure		400		{object}	ErrorResponse
//	@Router			/api/rcon/direct [post]
func sendDirectRcon(c *gin.Context) {
	var req DirectRconRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	if req.Command == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "command is empty"})
		return
	}
